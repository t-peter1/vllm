...
#
# This file was adapted for DCLM support.
#
...
"""Inference-only DCLM model compatible with HuggingFace weights."""
...
class DCLMDecoderLayer(nn.Module):
    def __init__(
        self,
        vllm_config: VllmConfig,
        prefix: str = "",
        config: LlamaConfig | None = None,
        attn_layer_type: type[nn.Module] = DCLMAttention,
    ) -> None:
        super().__init__()
...
class DCLMModel(nn.Module):
    def __init__(
        self,
        *,
        vllm_config: VllmConfig,
        prefix: str = "",
        layer_type: type[nn.Module] = DCLMDecoderLayer,
    ):
        super().__init__()
        ...
        if not hasattr(config, "max_position_embeddings"):
            config.max_position_embeddings = getattr(config, "seq_len", 2048)
...
class DCLMForCausalLM(
    ...
    def _init_model(
        ...
        return DCLMModel(vllm_config=vllm_config, prefix=prefix, layer_type=layer_type)
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for name, loaded_weight in weights:
            if "output" in name or "lm_head" in name:
                ...
                continue
            params_loaded_by_model = self.model.load_weights([(name, loaded_weight)])
            loaded_params.update(params_loaded_by_model)
            
        return loaded_params


class DCLMBidirectionalForSequenceClassification(as_seq_cls_model(DCLMForCausalLM)):
    # This class sets the correct attention type and pooling type
    # through LlamaBidirectionalConfig.
    pass


class DCLMBidirectionalModel(as_embedding_model(DCLMForCausalLM)):
    # This class sets the correct attention type and pooling type
    # through LlamaBidirectionalConfig.
    pass
